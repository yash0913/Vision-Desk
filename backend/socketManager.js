  ...
  io.on('connection', (socket) => {
    ...
    socket.on("register-device", ({ deviceId, userId, type }) => {
      try {
        ...
        // Track in online devices mapping for emitToDevice function
        trackUserSocket(onlineDevicesById, devId, socket.id);

        console.log("[REGISTERED DEVICE]", devId, deviceType, "for user", userIdStr);
        console.log("[DEVICE REGISTRY KEYS]", Array.from(deviceRegistryById.keys()));

        // Flush pending signals if any
        const pending = pendingSignalsByDevice.get(devId);
        if (pending && pending.length > 0) {
          console.log(`[REGISTER-DEVICE] Flushing ${pending.length} pending signals to ${devId}`);
          for (const sig of pending) {
            try {
              socket.emit(sig.event, sig.payload);
            } catch (e) {
              console.error('[REGISTER-DEVICE] Error flushing pending signal:', e);
            }
          }
          pendingSignalsByDevice.delete(devId);
        }
      } catch (err) {
        console.error('[REGISTER-DEVICE] Error:', err && err.message);
      }
    });
